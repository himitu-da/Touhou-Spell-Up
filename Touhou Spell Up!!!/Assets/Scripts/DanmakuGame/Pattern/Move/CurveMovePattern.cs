...
{
    [Header("移動設定")]
    [SerializeField] private FloatReference speed = new FloatReference { useConstant = true, constantValue = 5f };
    [SerializeField] private FloatReference angleChangeRate = new FloatReference { useConstant = true, constantValue = 30f }; // 度/秒
    ...
    public override async UniTask ExecuteMove(MovementState state, CancellationToken token)
    {
        ...
        {
            state.Rotation = Quaternion.Euler(0, 0, initialDirection.Value);
        }
